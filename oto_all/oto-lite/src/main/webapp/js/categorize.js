...
var used_term_counts = new Object();
			...
			XHR.onreadystatechange = function() {
				if (XHR.readyState == 4) {
					if (XHR.status == 200) {
						...
						alert("Category '" + categoryName
								+ "' was not deleted. Please try again later. ");
					}
				}
			};
...
function getGroupClone() {
	...
	for (i = term_rows.length - 1; i >= 0; i--) {
		var elements = term_rows[i].getElementsByTagName("td");
		var j;
		for (j = 0; j < elements.length; j++){
			elements[j].getElementsByTagName("label")[0].style.color = "black";
		}
	}
...
function update_group_chosen() {
	if (drag_from == "right") { //if dragging from left to right, do not remove from Terms list. --ags
		var i;
			...
			if (dragGroupTable) { //current_obj.className == "termLabel dragme" || current_obj.className == "decisionRemoved termLabel dragme") {// drag before saving
				if (target_category != null) {// (left -> target category) or
					...
					for (j = termlist.length - 1; j >= 0; j--) {
						update_used_term_count("subtract", termlist[j].id);
					}
					
					// append selected terms
					//append_selected_terms(document.getElementById("availableTerms")); //comment out because if terms were never removed from left side, we don't want duplicates. --ags
					setDragMeSign("drag");
...
function list_contains_element(element){
	var termlist = target_category.getElementsByClassName('term_row');
	
	var i;
	for (i = 0; i < termlist.length; i++) {
		if (termlist[i].id == element.id){
			return true;
		}
	}
	return false;
...
function update_used_term_count(operation, termname){
	//if there is no entry for 'term' int the table, create one. 
	if (typeof used_term_counts[termname] === 'undefined')
		used_term_counts[termname] = 0;
	//increment or decrement this term's count.
	if (operation == "add" && drag_from == "left"){
		used_term_counts[termname]++;
	} else if (operation == "subtract"){
		used_term_counts[termname]--;
	}
	if (used_term_counts[termname] > 0)
		document.getElementById(termname).getElementsByTagName('label')[0].style.color = "gray";
	else
		document.getElementById(termname).getElementsByTagName('label')[0].style.color = "black";
	
	//alert(operation + " " + termname + ". Count: " + used_term_counts[termname]);
}
...
function append_selected_terms(target) {
	...
	if (group_existed) {
		...
		for (j = termlist.length - 1; j >= 0; j--) {
			if (!list_contains_element(termlist[j])){ // don't allow for duplicates. --ags
				update_used_term_count("add", termlist[j].id);
			...
			if (XHR.readyState == 4) {
				if (XHR.status == 200) {
					if (XHR.responseText == "success") {
						...
						alert("Your changes were not saved. Please try again later. ");
					}
				} else {
					alert("Your changes were not saved. Please try again later. ");
				}
...
function fixTypo(term) {
	...
	var replacement = prompt("Edit term '" + term
			+ "':", "");
	if (!replacement) {
		return;
	}
	...
	var confirmed = confirm("You are about to correct term '" + term + "' to '"
			+ replacement + "'. "
			+ "This will affect all copies of the term. \n"
...
window.onbeforeunload = function() {
	...
	if (dataChanged()) {
		return ("You have unsaved changes. "
				+ "If you leave the page you will lose both your reviewing history and categorizing decisions.");
